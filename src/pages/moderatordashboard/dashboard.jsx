  ...
  return (
    <div className="flex flex-col h-screen bg-white text-gray-900">
      ...
      <img src={Wheel} alt="Logo" width={100} height={50} />
        ...
        <h1 className="flex text-xl text-orange-600 font-bold">Moderator Dashboard</h1>
        <button onClick={handleLogout} className="flex items-center bg-white px-4 py-2 rounded-md hover:bg-red-600 hover:text-amber-100 cursor-pointer">
          <LogOut size={18} className="mr-2" /> Logout
        </button>
      </nav>

      <div className="flex flex-1 mt-32"> {/* Push content below navbar */}
        {/* Sidebar */}
        <aside className={`fixed inset-y-0 left-0 w-64 bg-orange-500 text-white p-4 shadow-md transform ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        } transition-transform md:relative md:translate-x-0`}>
          <button onClick={() => setSidebarOpen(false)} className="md:hidden text-white text-right block w-full">
            <X size={24} />
          </button>
          <nav>
            <ul className="space-y-2">
              <li className="p-2 bg-orange-600 rounded text-center">Dashboard</li>
              <li className="p-2 hover:bg-orange-700 rounded cursor-pointer text-center">Users</li>
              <li className="p-2 hover:bg-orange-700 rounded cursor-pointer text-center">Reports</li>
            </ul>
          </nav>
        </aside>
        ...
        <div className="flex-1 p-6 overflow-auto">
          <div className="bg-white p-4 rounded-md shadow-md">
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {comments.map(({ _id, user, text, status }) => (
